...
import (
	...
	"github.com/jakubartur/open-ethereum-pool/payouts"
	"github.com/jakubartur/open-ethereum-pool/rpc"
	"github.com/jakubartur/open-ethereum-pool/storage"
	"github.com/jakubartur/open-ethereum-pool/util"
)
...
func (s *ApiServer) listen() {
	r := mux.NewRouter()
	r.HandleFunc("/api6/stats", s.StatsIndex)
	r.HandleFunc("/api6/stats/{chart:charts?}", s.StatsIndex)
	r.HandleFunc("/api6/miners", s.MinersIndex)
	r.HandleFunc("/api6/blocks", s.BlocksIndex)
	r.HandleFunc("/api6/payments", s.PaymentsIndex)
	r.HandleFunc("/api6/settings", s.Settings)
	r.HandleFunc("/api6/accounts/{login:0x[0-9a-fA-F]{40}}", s.AccountIndex)
	r.HandleFunc("/api6/accounts/{login:0x[0-9a-fA-F]{40}}/{chart:charts?}", s.AccountIndex)
